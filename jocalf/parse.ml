...
let unexpected_error msg lexbuf =
  failwith
    ( "Unexpected parsing exception: " ^ msg ^ "\noccurred at "
    ^ location_message lexbuf )

let parse parser_start s =
  let lexbuf = from_string s in
  try parser_start Lexer.token lexbuf with
  | Parser.Error | Lexer.Error -> parse_error lexbuf
  | Failure s -> unexpected_error s lexbuf

let parse_expr = parse Parser.parse_expression

let parse_phrase = parse Parser.parse_phrase
